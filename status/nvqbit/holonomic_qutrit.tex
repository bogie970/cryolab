...
holonomy~\cite{Neef2023}. % Neef2023 p.1 Abstract (photonic waveguide system with 3D degenerate dark subspace; first experimental U(3) holonomy)
...
$m_s = -1$ manifolds are separated by $D \approx 2.87$~GHz~\cite{Doherty2013}, and mixing
...
The key insight from Vitanov (2012)~\cite{Vitanov2012} is that any $\SU(3)$ transformation % Vitanov2012 Sec. III.A Eq. 12: Givens decomposition theorem
...
transformation requires at most 9 parameters % Vitanov2012 Sec. III.A, after Eq. 13: "9 consecutive interaction steps are required"
(matching $\dim[\SU(3)] = 8$ % Vitanov2012 Sec. II.B.2, after Eq. 8: "the total number of real independent parameters in Eq. (8) is eight, exactly as needed"
...
\textbf{9 consecutive interaction steps} are needed. % Vitanov2012 Sec. II, Theorem 1
Alternatively, using quantum
Householder reflections, any $\SU(3)$ can be realized in \textbf{3 reflections, each
requiring a single interaction step}, reducing the total to 3 steps~\cite{Vitanov2012}. % Vitanov2012 Sec. III, Theorem 2
...
30$--$300~\mu$s, well within the nuclear $T_2 \approx 10$~ms
...
the slower nuclear Rabi frequencies ($\sim 30$~kHz) mean that ``fast'' non-adiabatic gates
...
\begin{itemize}
  ...
  \item \textbf{$T_2$ (nuclear):} $\approx 10$~ms in NV$^-$~\cite{Pfender2017}. This
    applies to \emph{all} nuclear coherences ($m_I = 0 \leftrightarrow \pm 1$ and
...
Coherence time ($T_2^{(\text{nuc})}$) & $\approx 10$~ms & $\approx 10$~ms \\
...
nuclear $T_2 \approx 10$~ms gives $\sim 33$--$333\times$ headroom even for a full
$\SU(3)$ gate. Decoherence is \textbf{not a showstopper} for the qutrit encoding.
...
10$~ms vs.\ electron $T_2 \sim \mu$s) and coupling strengths differ substantially.
...
\begin{enumerate}[label=(\alph*)]
  \item \textbf{Neef et al.\ (2023)}~\cite{Neef2023}: First experimental realization % Neef2023 Nat. Phys. 19, 30--34; described in companion paper (Neef2023_3D_NonAbelian_Holonomy.pdf) p.4: "realization of an adiabatic holonomy belonging to the U(3) group"
    ...
    % PDF obtained: Neef2023_3D_NonAbelian_Holonomy.pdf (arXiv companion, 2026-03-19)
...
\begin{itemize}
  \item \cite{Neef2023}: Neef et al.\ (2023), Nat.\ Phys.\ \textbf{19}, 30--34 [U(3) holonomy, photonic] ---
    PDF obtained as \texttt{Neef2023\_3D\_NonAbelian\_Holonomy.pdf} (arXiv companion paper).
    Source locators added at lines of use. % Neef2023 key result: p.4 of companion "realization of an adiabatic holonomy belonging to the U(3) group"
...
\bibitem{Neef2023}
V.~Neef, J.~Pinske, \textit{et al.},
``Three-dimensional non-Abelian quantum holonomy,''
Nat.\ Phys.\ \textbf{19}, 30--34 (2023).
% PDF in NVQbit/literature/Neef2023_3D_NonAbelian_Holonomy.pdf (arXiv companion paper; obtained Task \#124, Cycle 37, 2026-03-20)
