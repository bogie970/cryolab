...
$m_s = -1$ manifolds are separated by $D \approx 2.87$~GHz~\cite{Doherty2013}, % Doherty2013 p.23 Eq.1 (NV ground-state ZFS D_gs ~ 2.87 GHz at RT; H_gs = D_gs[S_z^2 - S(S+1)/3] + ...) and mixing
...
Experimental validation of the hyperfine tripod holonomic qubit protocol on the NV center platform
is provided by the UTA PI group companion paper~\cite{Biswas2026}, which demonstrates that the
sequential adiabatic loop approach is compatible with the $^{14}$N hyperfine tripod structure and
supports the Route~B recommendation for NVQbit. % Biswas2026 (UTA PI group, unpublished ms. 2026): companion paper validating hyperfine tripod holonomic protocol; supports Route B sequential holonomic gate strategy
...
the slower nuclear Rabi frequencies ($\sim 30$~kHz) % Smeltzer2009 Fig.2j (^14N Rabi nutation, Omega=32.9 kHz; ms=-1 manifold, B=509G) mean that ``fast'' non-adiabatic gates
...
B.~Smeltzer, J.~McIntyre, and L.~Childress,
``Robust control of individual nuclear spins in diamond,''
Phys.\ Rev.\ A \textbf{80}, 032322 (2009).
% PDF: NVQbit/literature/Smeltzer_2009_Nuclear_Spin_Control.pdf; arXiv:0909.3896v1
% Fig.~2j: $^{14}$N Rabi nutation ($m_s=-1$, $B=509$\,G), $\Omega=32.9$\,kHz
...
Unpublished manuscript, UT Arlington (2026).
% UTA PI group companion paper on hyperfine tripod holonomic qubit protocol; validates Route B recommendation.
% PDF IN HAND: C:\Users\jbogi\Documents\claudeanalizer\NVQbit\literature\Biswas2026_GeometricPhase_NV_Hyperfine_Tripod.pdf
% Also present: Biswas2026_GeometricPhase_NV_IEEE.pdf, Biswas2026_GeometricPhase_NV.pdf (same folder)
